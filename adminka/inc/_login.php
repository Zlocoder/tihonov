...
<?PHP
	...
	$lmail = $func->IsMail($_POST["log_email"]);
		...
		if($lmail !== false){
			...
			$db->Query("SELECT id, user, pass, referer_id, banned FROM netdostupa_users_a WHERE email = '$lmail'");
			if($db->NumRows() == 1){
			...
			$log_data = $db->FetchArray();
				...
				if(strtolower($log_data["pass"]) == strtolower($_POST["pass"])){
					...
					if($log_data["banned"] == 0){
						...
						# Считаем рефералов
						$db->Query("SELECT COUNT(*) FROM netdostupa_users_a WHERE referer_id = '".$log_data["id"]."'");
						$refs = $db->FetchRow();
